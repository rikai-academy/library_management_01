...
<div class="container-fluid">
    <!-- Page Heading -->
    @if ($status == 1)
        <h1 class="h3 mb-2 text-gray-800">{{__('message.book_borrow')}}</h1>
    @elseif ($status == 2)
        <h1 class="h3 mb-2 text-gray-800">{{__('message.book_borrow_waiting')}}</h1>
    @elseif ($status == 3)
        <h1 class="h3 mb-2 text-gray-800">{{__('message.book_borrow_return')}}</h1>
    @endif
    <!-- DataTales Example -->
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th>{{__('message.datetime_borrow')}}</th>
                            <th>{{__('message.datetime_return')}}</th>
                            <th>{{__('message.sub_total')}}</th>
                            <th>{{__('message.action')}}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @include('Admin.RentalBook.Elements.list_user_rental')
                    </tbody>
                </table>
            </div>
        </div>
